...
public class BrainScrolling : MonoBehaviour
{
    ...
    [SerializeField]  Text bayBtnText;
    ...
    private void Start()
    {
        ...
        {
            instPans[i] = Instantiate(brainPrefab, transform, false);
            instPans[i].GetComponent<Image>().sprite = customSystem.customsSprites[i];
            if (i == 0) continue;

            var tmpX = instPans[i - 1].transform.localPosition.x + panPrefab.GetComponent<RectTransform>().sizeDelta.x + panSpace;
